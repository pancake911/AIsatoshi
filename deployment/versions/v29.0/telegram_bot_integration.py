...
class AIsatoshiTelegramBot:
    ...
    def _send_single_message(self, chat_id: str, text: str, parse_mode: str = None, save_to_db: bool = True) -> bool:
        """发送单条消息并保存到数据库"""
        try:
            data = {
                'chat_id': chat_id,
            ...
            # 只在 parse_mode 不是 None 时才发送该字段（修复 parse_mode 错误）
            ...
            if response.status_code == 200:
                ...
                self.logger.warning(f"Telegram API 返回错误: {response.status_code} - {response.text[:200]}")
            return False
        except Exception as e:
            self.logger.error(f"发送消息失败: {e}")
            return False

    def send_message(self, chat_id: str, text: str, parse_mode: str = None, save_to_db: bool = True) -> bool:
        """发送消息并保存到数据库 - 支持分段发送长消息"""
        try:
            # 分段发送（每段3000字符，留余量避免超过Telegram 4096限制）
            max_length = 3000
            if len(text) > max_length:
                self.logger.info(f"[分段发送] 消息长度 {len(text)} 字符，将分 {(len(text) + max_length - 1) // max_length} 段发送")

                # 第一段
                chunk = text[:max_length]
                self._send_single_message(chat_id, chunk, parse_mode, save_to_db)
                ...
                for i in range(1, num_chunks):
                    ...
                    # 只在最后一段保存到数据库，避免重复
                    self._send_single_message(chat_id, chunk, parse_mode, save_to_db and (i == num_chunks-1))
                return True

            # 短消息直接发送
            return self._send_single_message(chat_id, text, parse_mode, save_to_db)
        except Exception as e:
            self.logger.error(f"发送消息失败: {e}")
            return False
